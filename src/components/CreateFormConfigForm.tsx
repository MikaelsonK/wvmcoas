...
export function CreateFormConfigForm({ domains }: { domains: DomainOption[] }) {
  ...
  return (
    <Form ref={formRef} action={action} validationErrors={state.errors} className="flex flex-col gap-4">
          ...
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={i} className="flex gap-3 flex-wrap items-start px-4 py-3 bg-gray-50/50">
              ...
              <Select name={`qType_${i}`} defaultSelectedKey="single_select" className="w-52 flex flex-col gap-1">
                ...
                <Button className="w-full flex items-center justify-between px-3.5 py-2.5 text-[13px] text-gray-900 bg-gray-50 border border-gray-200 rounded-lg outline-none transition-all duration-150 focus:bg-white focus:border-brand-red focus:ring-2 focus:ring-brand-red/10 cursor-default text-left data-[hovered]:bg-gray-100/50">
                  <SelectValue className="block truncate data-[placeholder]:text-gray-400" />
                  <span aria-hidden="true" className="text-gray-400 text-xs">▼</span>
                </Button>
                <Popover className="w-[var(--trigger-width)] max-h-60 overflow-auto rounded-md border border-gray-200 bg-white shadow-lg focus:outline-none z-50">
                  <ListBox className="p-1 focus:outline-none">
                    <ListBoxItem id="single_select" className="cursor-default rounded px-2.5 py-2 text-[13px] outline-none data-[focused]:bg-gray-100 data-[selected]:bg-brand-red data-[selected]:text-white text-gray-800 transition-colors duration-100">
                      Single Select (Numeric)
                    </ListBoxItem>
                    <ListBoxItem id="multi_select" className="cursor-default rounded px-2.5 py-2 text-[13px] outline-none data-[focused]:bg-gray-100 data-[selected]:bg-brand-red data-[selected]:text-white text-gray-800 transition-colors duration-100">
                      Multi Select
                    </ListBoxItem>
                    <ListBoxItem id="text" className="cursor-default rounded px-2.5 py-2 text-[13px] outline-none data-[focused]:bg-gray-100 data-[selected]:bg-brand-red data-[selected]:text-white text-gray-800 transition-colors duration-100">
                      Text Response
                    </ListBoxItem>
                  </ListBox>
                </Popover>
      ...
      <Button
        ...
        className="self-start px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-brand-red hover:bg-[#8a0606] disabled:opacity-50 transition-colors duration-150 cursor-default text-center"
      >
