...
export function CreateProcedureForm({ categories, onSuccess }: { categories: CategoryOption[]; onSuccess?: () => void }) {
      ...
      <Select isRequired name="procedureTypeId" className="flex flex-col gap-1.5">
        ...
        <Popover className="w-[var(--trigger-width)] max-h-60 overflow-auto rounded-md border border-gray-200 bg-white shadow-lg focus:outline-none z-50">
          <ListBox className="p-1 focus:outline-none">
            ...
            {categories.map((t) => (
              <ListBoxItem key={t.id} id={t.id} className="cursor-pointer rounded px-2.5 py-2 text-sm outline-none data-[focused]:bg-gray-100 data-[selected]:bg-brand-red data-[selected]:text-white text-gray-800 transition-colors duration-100">
                {t.name}
              </ListBoxItem>
            ))}
          </ListBox>
...
export function CreateProcedureModalTrigger({ categories }: { categories: CategoryOption[] }) {
  ...
  return (
    <>
      ...
      <DialogModal isOpen={isOpen} onOpenChange={setIsOpen} title="Create Procedure">
        <CreateProcedureForm categories={categories} onSuccess={() => setIsOpen(false)} />
      </DialogModal>
    </>
  );
}
